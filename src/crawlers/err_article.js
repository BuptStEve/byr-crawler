 ...
 * @Last modified time: 2016-Jul-31 01:25:02
 */
...
function saveComments(article, $elet, tmpComments, $) {
  const rank = $elet.find('div.nav a.plant').eq(0).text().split('楼')[0]; // 楼数
  const author = $elet.find('div.nav a').eq(1).text(); // 回帖人
  const cmtTime = new Date($elet.find('div.nav a.plant').eq(1).text()); // 回帖时间
  const $comment = $elet.find('div.sp');
  const cmtBody = addBaseUrl2Img(Config.url.index, $comment, $); // 回帖内容(html)
  ...
  superagent
    ...
    .end((err, sres) => {
      ...
      if ($('#m_main > ul').length === 0) {
        ...
        const articleBody = addBaseUrl2Img(Config.url.index, $article, $); // 处理图片 src 后的主贴 html
 ...
 * @author BuptStEve
 */
function updateErrorArticles(cookie, next) {
  console.time('updateErrorArticles');
  ...
  function mainProcess(isLast, cookie, next) {
    async.waterfall([
       ...
       * @author BuptStEve
       */
      function getErrorPage(articles, next) {
        const articlePage = [];

        async.eachLimit(articles, 1, (article, callback) => {
          console.log(article.url);
          if (article.url.lastIndexOf('?p=') === -1) {
            // 第一页
            ...
            // 非第一页
            getOneArticlePage(article, cookie, err => {
              if (err) return callback(err);
      ...
      if (err) return next(`updateErrorArticles err: ${err}`);
      ...
      if (!isLast) {
        ...
        console.timeEnd('updateErrorArticles');
        next(null);
      }
